# ---------------------------------------------------------------------------------------
# Program           : RFunction_correlation-plot_corrplot-modified.R
# Modified from     : PRS_UKB_201711_step18-06-01_function_correlation_plot.R
# Author            : Taiyun Wei
# Date created      : 20180920
# Purpose           : Tweak corrplot::corrplot() source code to expand tl.col to 2 vectors, one for coloring verical dimension of the heatmap, the other for coloring the horizontal dimension of the heatmap
# Note              : The function parameter is.corr= is not understandable. Set this to TRUE if showing numbers in cells. Set this to FALSE when showing values as circles
...
# Function internal : PlotHeatmapByCorrplot(),CallFunctionPlotHeatmapByCorrplot()
...
# Sys.Date() History
#----------------------------------------------------------------------------------------
# 20180920   Named functions and parameters following Google's R style guide https://google.github.io/styleguide/Rguide.xml
# 20180920   Saved this file to RFunction folder. Changed function name from f_corrplot() to PlotHeatmapByCorrplot()  
#----------------------------------------------------------------------------------------

#----------------------------------------------------------
# Folder locations under software
#----------------------------------------------------------
loc.R.library <- "/software/R/R-3.4.1/lib64/R/library"
...
library(corrplot, lib.loc = loc.R.library) # function "colorlegend" depends on this package

DrawGrid<-function (coords, fg){
  symbols(coords, add = TRUE, inches = FALSE, fg = fg, bg = NA, 
          rectangles = matrix(1, nrow = nrow(coords), ncol = 2))
}

PlotHeatmapByCorrplot <-function (corr
                                  ...
                                  ,bg = "white"
                                  ,title = ""
                                  ,is.corr = TRUE # For general matrix, please using is.corr = FALSE to convert.
                                  ,diag = TRUE
                                  ,outline = FALSE
                                  ,mar = c(0, 0, 0, 0)
                                  ,addgrid.col = NULL
                                  ...
                                  ,tl.pos = NULL
                                  ,tl.cex = 1
                                  ,tl.col.horizontal.vector = NULL
                                  ,tl.col.vertical.vector = NULL
                                  ,tl.offset = 0.4
                                  ,tl.srt = 90
                                  ,cl.pos = NULL
                                  ,cl.lim = NULL
                                  ...
                                  ,na.label.col = "black"
                                  ,win.asp = 1, ...) {
  # Tweak source code of corrplot::corrplot() to change colors of vertical and horizontal dimension of the exported heatmap. The following code is the source code with tl.col replaced by tl.col.horizontal.vector and tl.col.vertical.vector
  ...
  DrawGrid(AllCoords, addgrid.col)
  if (plotCI != "n") {
  ...
  if (tl.pos != "n") {
    ...
    if (tl.pos == "d") {
      ...
      text(pos.ylabel[, 1] + 0.5, pos.ylabel[, 2], newcolnames[1:min(n, 
                                                                     m)], col = tl.col.horizontal.vector, cex = tl.cex, ...)
    }
    else {
      text(pos.xlabel[, 1], pos.xlabel[, 2], newcolnames, 
           srt = tl.srt, adj = ifelse(tl.srt == 0, c(0.5, 
                                                     0), c(0, 0)), col = tl.col.horizontal.vector, cex = tl.cex, 
           offset = tl.offset, ...)
      text(pos.ylabel[, 1], pos.ylabel[, 2], newrownames, 
           col = tl.col.vertical.vector, cex = tl.cex, pos = 2, offset = tl.offset, 
           ...)
    }
  }
  title(title, ...)
...
#-------------------------------------------------------------------------
# Make a function for making a heatmap using f_corrplot()
#-------------------------------------------------------------------------
CallFunctionPlotHeatmapByCorrplot <-function(output.file.path 
                                             ,plot.width 
                                             ,plot.height 
                                             ,plot.data.matrix
                                             ,plot.method
                                             ...
                                             ,cex.number
                                             ,ordering.method 
                                             ,num.rectangles.drew.hierarchical.cluster 
                                             ,color 
                                             ,text.label.horizontal.color 
                                             ,text.label.vertical.color 
                                             ,R2.lowerBound 
                                             ,R2.upperBound 
                                             ,plot.label.cex 
                                             ,plot.data.signi.matrix 
                                             ,plot.data.signi.threshold 
                                             ,colorlegend.position.x 
                                             ,colorlegend.position.y 
                                             ,colorlegend.cex
                                             ,YAxisTitle
                                             ,XAxisTitle){
  # Call the function PlotHeatmapByCorrplot() above.
  # 
  # Args:
  #   output.file.path: Output plot file path named by users
  #   plot.width:  Width of output plot 
  #   plot.height: Height of output plot 
  #   plot.data.matrix: A matrix containing values to plot
  #   plot.method
  #   plot.type:  must be one of c("full", "lower", "upper")   
  #   correlation.or.not: Set is.corr to TRUE or FALSE
  #   ordering.method: One of order argument of PlotHeatmapByCorrplot()  
  #   num.rectangles.drew.hierarchical.cluster 
  #   color: 10 gradient colors between two colors (e.g. red to yellow)
  #   text.label.horizontal.color: Colors for horizontal labels defined by users
  #   text.label.vertical.color: Colors for vertical labels defined by users
  #   R2.lowerBound: User-defined lower bound of R square. This value should be slightly smaller than the minimal of plot values
  #   R2.upperBound: User-defined upper bound of R square. This value should be slightly greater than the maximum of plot values
  #   plot.label.cex: Font size of vertical and horizontal label text 
  #   plot.data.signi.matrix: A matrix containing p values for each value of the matrix to plot 
  #   plot.data.signi.threshold: A single cutoff value for determining the p value as significant or non-significant. Plot values are not plotted if their p values are non-significant 
  #   colorlegend.position.x: Left and right position of the color legend defined as c(x1,x2)
  #   colorlegend.position.y: Bottom and top position of the color legend defined as c(y1,y2)
  #   colorlegend.cex: Font size of text in color legend
  #
  # Returns:
  #   A PNG file named by output.file.path
  
  #library(corrplot)
  
  png(file=output.file.path, width= plot.width, height=plot.height)
  
  par(mfrow=c(1,1)
      ,mar=c(2,22,2,4)
      ,oma=c(2,2,2,3))
  
  PlotHeatmapByCorrplot(corr=plot.data.matrix
                        ,method = plot.method #"circle"
                        ,type = plot.type #"full"
                        ,col =color #gradient_10colors_dark_to_light
                        ,bg = "white"
                        ,is.corr = correlation.or.not
                        ,diag = TRUE
                        ,outline = FALSE
                        , mar = c(0, 0, 0, 0)
                        , addgrid.col = NULL
                        , addCoef.col = NULL
                        , addCoefasPercent = FALSE
                        , order = ordering.method #"original"
                        , addrect = num.rectangles.drew.hierarchical.cluster 
                        , rect.col = "black"
                        , rect.lwd = 2
                        , tl.pos = NULL # text label position
                        , tl.cex = plot.label.cex #6
                        , tl.col.horizontal.vector = text.label.horizontal.color
                        , tl.col.vertical.vector= text.label.vertical.color
                        , tl.offset = 0.4
                        , tl.srt = 90
                        , cl.pos= "n" # position of color legend (labels). cl.pos="n" draws no colorlegend
                        , cl.lim = c(R2.lowerBound,R2.upperBound) # The limits (x1, x2) in the 'c'olor'l'abel.
                        , cl.length = NULL
                        , cl.cex = 0.8
                        , cl.ratio = 0.15
                        , cl.align.text = "c"
                        , cl.offset = 0.5
                        , number.cex = cex.number
                        , number.font = 2
                        , number.digits = NULL
                        , addshade = c("negative", "positive", "all")
                        , shade.lwd = 1
                        , shade.col = "white"
                        , p.mat=plot.data.signi.matrix # Matrix of p-value, if NULL, arguments sig.level, insig, pch, pch.col, pch.cex is invalid.
                        , sig.level = plot.data.signi.threshold
                        , insig = "blank"
                        , pch = 4 # mark nonsignificant values as pch=4 (X), if argument has no effect when insig="blank"
                        , pch.col = "black"
                        , pch.cex = 10
                        #, plotCI = c("n", "square", "circle", "rect")
                        , lowCI.mat = NULL
  ...
  # Customise the color legend
  colorlegend(color
              , xlim=colorlegend.position.x #c(1,35) # x positions of left and right border, forming the width the legend
              , ylim=colorlegend.position.y #c(35,39) # y positions of bottom and top border, forming the height of the legend
              , c(seq(from=R2.lowerBound,to= R2.upperBound,by=(R2.upperBound-R2.lowerBound)/10)) # labels
              , align="l" # alignment of labels, "l" means left, "c" means center and "r" right.
              , vertical=F # put
              , addlabels=TRUE
              , cex=colorlegend.cex
              ) 
  ...
  # Add y axis title
  mtext(side=2,YAxisTitle,line=10,cex=12)
  
  # Add x axis title
  mtext(side=3,XAxisTitle,line=-75,cex=12)
  
  # Add R-squared beside the colorlegend
  legend(x=35.5,y=0,legend=bquote(R^2),bty="n",xpd=NA, cex=12) # legend=bquote(R^2)
  
  dev.off()
}

# Copy this script for similar work
#from=paste0(locScripts,"PRS_UKB_201711_step18-06-01_function_correlation_plot.R")
#to=paste0(locRFunction,"RFunction_correlation-plot_corrplot-modified.R")