##################################################################################
# filename: RFunction_GSMR_plots.R
# program author: GSMR team
# purpose: 
# Reference: http://cnsgenomics.com/software/gsmr/
...
# ************************************************** #
gsmr_snp_effect = function(gsmr_data=NULL, expo_str=NULL, outcome_str=NULL) {
...
plot_snp_effect = function(expo_str
                           , expo_label
                           , outcome_str
                           , outcome_label
                           , bxy
  ...
  plot(bzx
       ,bzy
       ,pch=20
       ,cex= plot_symbol_size # 0.8
       ,bty="n"
       ,cex.axis=axis_annot_size 
       ,cex.lab=axis_label_size 
       ,col=effect_col
       ,xlim=c(xmin, xmax)
       ,ylim=c(ymin, ymax)
       #,xlab=substitute(paste(trait, " (", italic(b[zx]), ")", sep=""), list(trait=expo_str))
       ,xlab=substitute(paste(trait, " (", italic(b[zx]), ")", sep=""), list(trait=expo_label))
       #,ylab=substitute(paste(trait, " (", italic(b[zy]), ")", sep=""), list(trait=outcome_str))
       ,ylab=substitute(paste(trait, " (", italic(b[zy]), ")", sep=""), list(trait=outcome_label))
       )
...
# ************************************************** #
plot_snp_pval = function(expo_str
                         , expo_label
                         , outcome_str
                         , outcome_label
                         , bzx_pval
  ...
  plot(bzx_pval
       ...
       , cex=plot_symbol_size #0.8
       , bty="n"
       , cex.axis=axis_annot_size #3
       , cex.lab=axis_label_size #3
       , col=effect_col
       , xlim=c(min_val, max_val)
       , ylim=c(min_val, max_val)
       #, xlab=substitute(paste(trait, " (", -log[10], italic(P)[zx], ")", sep=""), list(trait=expo_str))
       , xlab=substitute(paste(trait, " (", -log[10], italic(P)[zx], ")", sep=""), list(trait=expo_label))
       #, ylab=substitute(paste(trait, " (", -log[10], italic(P[zy]), ")", sep=""), list(trait=outcome_str)))
       , ylab=substitute(paste(trait, " (", -log[10], italic(P[zy]), ")", sep=""), list(trait=outcome_label)))
...
# ************************************************** #
plot_snp_bxy = function(expo_str
                        , expo_label
                        , outcome_str
                        , outcome_label
                        , bxy
  ...
  plot(bxy
       ...
       , xlab=substitute(paste(italic(hat(b)[xy]), " (", trait1, " -> ", trait2, ")", sep=""), list(trait1=expo_label, trait2=outcome_label))
       #, ylab=substitute(paste(trait, " (", -log[10], italic(P[zx]), ")", sep=""), list(trait=expo_str)))
       , ylab=substitute(paste(trait, " (", -log[10], italic(P[zx]), ")", sep=""), list(trait=expo_label)))
}
...
plot_gsmr_effect = function(gsmr_data=NULL
                            , expo_str=NULL
                            , expo_label=NULL
                            , outcome_str=NULL
                            , outcome_label=NULL
...
# outcome_str, outcome
# effect_col, plotting colour
plot_gsmr_pvalue = function(gsmr_data
                            , expo_str
                            , expo_label
                            , outcome_str
                            , outcome_label
...
# outcome_str, outcome
# effect_col, plotting colour
plot_bxy_distribution = function(gsmr_data
                                 , expo_str
                                 , expo_label
                                 , outcome_str
                                 , outcome_label
