...
# path: "C:/Now/R/Rfunction_barPlot_xConti_yConti[1].R"
# plot type: overlapped line plots of different groups where x and y are numeric
# function name: barPlotGroup
# Sys.time()                Update  
#-------------------------------------------------------------------------------------------
#"2013-10-13 16:19:59 EST"  recode "in-hive" stage as "lost", remove "missing" stage, 
#                            use discrete X variable (FD_name) so easy to specify order of values on x axis
#
#-------------------------------------------------------------------------------------------
# status: working
#-------------------------------------------------------------------------------------------
...
barPlotGroup <- function(Data, Xvar="FD_order", Yvar="proportion_FD", Ymin=0, Ymax=1,BreakIncreY=0.2, 
  ...
  Fig01 <-ggplot(Data, aes_string(x=Xvar, y=Yvar, fill='trt_label')) + 
						...
						scale_x_discrete("stage", 
														 limits=c("lost", "orientation", "foraging"), #replace with c(1,2,3) if x values are numeric
														 labels=c("lost", "orientation", "foraging") #values to print on x axis
														)+
