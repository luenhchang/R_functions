...
# -----------------------------------------------------------------------------------------------
# File name     : RFunction_twinModelling_2VarACE-binary-binary[1]annotated.R
# Modified from : RFunction_twinModelling_2VarACE-binary-binary.R
# Purpose       : The original script that runs multivariate twin modelling with 3 ordinal and 1 binary dependent variables. This script reduces dependent variables to two, and number of thresholds to 1 for two binary dependent variables. A binary variable is simply an ordinal variable with just 1 threshold (i.e. 2 categories- 1 or 0).   
...
#------------------------------------------------------------------------------------------------------
# Import phenotype data in twin format
#------------------------------------------------------------------------------------------------------
...
#                                                  ,output.folder.path.pheno.gene.env.corr) {
  ...
  #   num.attempt.to.run.model:               Number of extraTries in mxTryHardOrdinal()
...
#---------------------------------------------------------------------------------------------
data <- data.pheno.here
dep.var  <- c("aspddx4","dsmiv_conductdx")
covariates <- c("sex","age")
var.suffix.twin1 <- "_01"
var.suffix.twin2 <- "_02"

dep.var.labels <- c("DSM4-antisocial-personality-disorder","DSM4-conduct-disorder") 
...
dep.var.twin1 <- paste0(dep.var,var.suffix.twin1) 

covariates.twin1 <- paste0(covariates,var.suffix.twin1)

dep.var.twin2 <- paste0(dep.var,var.suffix.twin2)
...
#---------------------------------------------------------------------------------------------------------------
# Check frequencies of dependent variables. Part of what’s slowing it up is that some of the cell frequencies are very spares. For example, run this:
#---------------------------------------------------------------------------------------------------------------

# Number of cases and controls of the 2 dependent variables of twin 1 who are MZ twins
freq.cases.controls.twin1.MZ <- table(mzdata[,selVars[1]],mzdata[,selVars[2]])

# Number of cases and controls of the 2 dependent variables of twin 2 who are MZ twins
freq.cases.controls.twin2.MZ <- table(mzdata[,selVars[3]],mzdata[,selVars[4]])

# Number of cases and controls of the 2 dependent variables of twin 1 who are DZ twins
freq.cases.controls.twin1.DZ <- table(dzdata[,selVars[1]],dzdata[,selVars[2]])

# Number of cases and controls of the 2 dependent variables of twin 1 who are DZ twins
freq.cases.controls.twin2.DZ <- table(dzdata[,selVars[3]],dzdata[,selVars[4]])

# Check if any of the 16 cells above contain zero. Only run the following code when each of them is non-zero
cell.freq.zero <- any(c(freq.cases.controls.twin1.MZ,freq.cases.controls.twin2.MZ,freq.cases.controls.twin1.DZ,freq.cases.controls.twin2.DZ) ==0) 
...
# $ dsmiv_conductdx_01: Ord.factor w/ 2 levels "0"<"1": 1 1 1 1 1 1 NA 1 1 1 ...
...
# $ dsmiv_conductdx_02: Ord.factor w/ 2 levels "0"<"1": NA NA 1 2 NA NA 1 1 1 1 ...
...
# $ dsmiv_conductdx_02: Ord.factor w/ 2 levels "0"<"1": 1 NA NA 1 1 NA 1 NA 1 1 ...
# $ sex_02            : num  2 -9 -9 2 1 -9 2 2 1 2 ...
# $ age_02            : num  36 -9 -9 36 35 -9 30 32 30 48 ...

# Run the following code when none of the 16 cells contain zero
if(cell.freq.zero=='FALSE'){
...
CholAceFit <- mxTryHardOrdinal(CholAceModel, extraTries=9, greenOK=T,checkHess=F,intervals=T)
...
# CholeskyACE.corrP[2,1] 0.8382821 0.8973353 0.9383056     
# CholeskyACE.corrP[1,2] 0.8382821 0.8973353 0.9383056     
# CholeskyACE.corrP[2,2]        NA 1.0000000        NA  !!!
# CholeskyACE.corrA[1,1]        NA 1.0000000        NA  !!!
# CholeskyACE.corrA[2,1]        NA 1.0000000        NA  !!!
# CholeskyACE.corrA[1,2]        NA 1.0000000        NA  !!!
# CholeskyACE.corrA[2,2]        NA 1.0000000        NA  !!!
# CholeskyACE.corrE[1,1]        NA 1.0000000        NA  !!!
# CholeskyACE.corrE[2,1] 0.2647927 0.6421378 0.9868437     
# CholeskyACE.corrE[1,2] 0.2647926 0.6421378 0.9827973     
...
CholAeFit <- mxTryHardOrdinal(CholAeModel, extraTries=9, greenOK=T,checkHess=F,intervals=T)
...
CholAeSumm$CI
#                          lbound  estimate    ubound note
# CholeskyAE.corrP[1,1]        NA 1.0000000        NA  !!!
# CholeskyAE.corrP[2,1] 0.8382821 0.8973352 0.9383049     
# CholeskyAE.corrP[1,2] 0.8382821 0.8973352 0.9383049     
# CholeskyAE.corrP[2,2]        NA 1.0000000        NA  !!!
# CholeskyAE.corrA[1,1]        NA 1.0000000        NA  !!!
# CholeskyAE.corrA[2,1]        NA 1.0000000        NA  !!!
# CholeskyAE.corrA[1,2]        NA 1.0000000        NA  !!!
# CholeskyAE.corrA[2,2]        NA 1.0000000        NA  !!!
# CholeskyAE.corrE[1,1]        NA 1.0000000        NA  !!!
# CholeskyAE.corrE[2,1] 0.2648109 0.6421374 0.9830471     
# CholeskyAE.corrE[1,2] 0.2648109 0.6421374 0.9823611     
# CholeskyAE.corrE[2,2]        NA 1.0000000        NA  !!!
...
CholAFit <- mxTryHardOrdinal(CholAModel, extraTries=9, greenOK=T,checkHess=F,intervals=T)
...
# CholeskyA.corrP[2,1] 0.8086546 0.8737088 0.9207277     
# CholeskyA.corrP[1,2] 0.8086546 0.8737088 0.9207277     
# CholeskyA.corrP[2,2]        NA 1.0000000        NA  !!!
# CholeskyA.corrA[1,1]        NA 1.0000000        NA  !!!
# CholeskyA.corrA[2,1] 0.9554880 1.0000000        NA  !!!
# CholeskyA.corrA[1,2] 0.9554880 1.0000000        NA  !!!
...
# 1 CholeskyAE       <NA> 10 1140.619 5349 -9557.381      NA     NA          NA
# 2 CholeskyAE  CholeskyA  9 1149.083 5350 -9550.917 8.46389      1 0.003622662
...
CholEFit     <- mxTryHardOrdinal(CholEModel, extraTries=9, greenOK=T,checkHess=F, intervals=T)
...
CholFit     <- mxTryHardOrdinal(CholModel, extraTries=9, , greenOK=T,checkHess=F, intervals=T)
...
CholAFit1     <- mxTryHardOrdinal(CholAModel1, extraTries=9, greenOK=T, checkHess=F, intervals=T)
...
CholEFit1     <- mxTryHardOrdinal(CholEModel1, extraTries=9, greenOK=T,checkHess=F, intervals=T)
...
CholFit1     <- mxTryHardOrdinal(CholModel1, extraTries=9, greenOK=T,checkHess=F, intervals=T)
...
#           ,na="" )
#---------------------------------------------------------------------------------------------------------
# End the if statement and the main function
#---------------------------------------------------------------------------------------------------------
} else {
  cat( "\n","WARNING: bivariate modelling is not run because zero found in the cell frequencies","\n"
      ,"Status of zeros = ", cell.freq.zero)
}  
